...
/**
	 ...
	 @param p un obstacle.
 */
void drawObstacle( Contexte* pCtxt, cairo_t* cr, Obstacle p );
...
/**
	...
	@param p probabilité (entre 0 et 1) qu'une particule soit effectivement créée.
	@param var donne la variabilité sur la vitesse initiale et sur la masse.
	@param x la coordonnée x de la position où la particule est créée.
	@param y la coordonnée y de la position où la particule est créée.
	@param vx la composante x de la vitesse de la particule.
	@param vy la composante y de la vitesse de la particule.
...
gboolean mouse_clic_reaction( GtkWidget *widget, GdkEventButton *event, gpointer data )
{
	...
	int y = event->y;
	Point point;
	point.x[0] = (double)x; point.x[1] = (double)y; 
	point = drawingAreaPoint2Point( pCtxt, point );
	
	Obstacle p;
	initObstacle( &p, DISQUE, point.x[0], point.x[1], 0.05, 0.6, 0.0, 0.0, 0.0 );
	TabObstacles_ajoute(&(pCtxt -> TabO), p);

	Detruire(pCtxt -> kdtree);
	pCtxt -> kdtree = KDT_Creer(pCtxt -> TabO.obstacles,0,TabObstacles_nb(&(pCtxt -> TabO)),0);
	return TRUE;
}
...
void drawObstacle( Contexte* pCtxt, cairo_t* cr, Obstacle p )
{
	...
	pp.x[ 0 ] = p.x[ 0 ];
	pp.x[ 1 ] = p.x[ 1 ];
	// On convertit les coordonnées réelles des obstacles (dans [-1:1]x[-1:1]) en coordonnées
	// de la zone de dessin (dans [0:499]x[0:499]).
	Point q = point2DrawingAreaPoint( pCtxt, pp );
	cairo_set_source_rgb (cr, p.cr, p.cg, p.cb);
	drawPoint( cr, q.x[ 0 ], q.x[ 1 ], length2DrawingAreaLength(pCtxt,p.r) );
}
											 ...
											 double x, double y, double vx, double vy, double m )
{
	TabParticules* P = &pCtxt->TabP;
	double random, newVx;
	if ( ( random = (rand() / (double) RAND_MAX) ) < p )
		{
			Particule q;
			newVx = random * ((1+var)*vx - (1-var)*vx) + (1-var)*vx;
			initParticule( &q, x, y, newVx, vy, m );
			TabParticules_ajoute( P, q );
		}
}
...
gint tic( gpointer data )
{
	...
	fontaineVariable( pCtxt, 30000, 0.1, -0.5, 0.5, 0.3, 0.3, 1.0 );
	fontaineVariable( pCtxt, 10000, 0.2, -0.5, 0.5, 0.3, 0.3, 0.5 );
	fontaineVariable( pCtxt, 60000, 0.18, -0.5, 0.5, 0.3, 0.3, 2.5 );
...
void deplaceParticule( Contexte* pCtxt, Particule* p )
{
	...
	for (int i = 0; i < DIM; i++) { p->x[ i ] += DT * p->v[ i ]; }
	...
	for ( int i = 0; i < TabObstacles_nb( ptrO ); ++i )
	{
		O = TabObstacles_get(ptrO,i);
		centre.x[0] = O.x[0];
		centre.x[1] = O.x[1];
		if (distance( centre.x[0], centre.x[1] , pp.x[0], pp.x[1] ) < O.r) { *p = calculRebond(*p,centre,O.r,O.att); }
	}
}
