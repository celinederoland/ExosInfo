...
int monTube_exo6_init(monTube_exo6_t *mt, int taille)
{
	...
	err = ftruncate(mt -> df, sizeof( monTube_exo6_partagee ));
	if (err != 0) 
	{
	...
	mt -> partage = (monTube_exo6_partagee*) mmap (NULL, sizeof(monTube_exo6_partagee), 
...
{
	int retour;
	if (mt -> partage -> ferme != 0 && mt -> partage -> nbElements == 0 ) { return 0; }
	retour = sem_wait(&(mt -> partage -> sem));
	//if (mt -> partage -> ferme != 0 && mt -> partage -> nbElements == 0 ) { return 0; }
...
int monTube_exo6_detruire(monTube_exo6_t *mt)
{
	...
	retour += munmap( mt -> partage, sizeof(monTube_exo6_partagee));
