...
# A la fin du fichier nous proposons des fonctions pour visualiser les résultats.#
# Pour voir les résultats exécutez test10(),test100(),test1000(). La fonction test10000() est plus longue à exécuter et prend un paramètre : poids1 ou poidsE #
...
# Explication : on obtient une figure approchant un 'camenbert' car les chemins obtenus par le parcours en profondeur peut être long et sinueux. Par conséquent, deux points géographiquement proches du départ peuvent être à des distances calculées très différentes selon les chemins empruntés. Ces chemins 'longs et sinueux', qui reviennent proches du point d'origine et s'en éloignent, se voient bien sur le graphe à 100 sommets. #
...
def test10():
	# print "Graphe créé en lisant les fichiers de points et de triangles" #
	G10 = lireTriangulation( "./pts-box-10.txt", "./tri-box-10.txt" )
	G10.show()

	print "Graphe à 10 sommets affiché plus lisiblement"
	P = afficheDiGraph( G10, 0.02 )
	P += text("graphe",(0,0),rgbcolor=(0,0,0))
	P.show( axes = False )

	print "Matrice poids pour le graphe à 10 sommets"
	...
	print "FloydWarshall pour le graphe à 10 sommets"
	(D10poids1, I10poids1) = floydWarshall( M10poids1 )
	print "poids 1:"
	print (D10poids1, I10poids1)
	(D10poidsE, I10poidsE) = floydWarshall( M10poidsE )
	print "poids E:"
	print (D10poidsE, I10poidsE)
	...
	print "Affichage des distances sur le graphe à 10 sommets"
	P = afficheDiGraphDistance( G10, 0, poids1, 0.02 )
	P += text("graphe distances 1",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 
	P = afficheDiGraphDistance( G10, 0, poidsE, 0.02 )
	P += text("graphe distances E",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 

	print "Affichage des distances du parcours en profondeur sur le graphe à 10 sommets"
	P = afficheDiGraphDistanceProfondeur( G10, 0, poids1, 0.02 )
	P += text("graphe profondeur 1",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 
	P = afficheDiGraphDistanceProfondeur( G10, 0, poidsE, 0.02 )
	P += text("graphe profondeur E",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 

def test100():
	# print "Graphe à 100 sommets affichés plus lisiblement" #
	G100 = lireTriangulation( "./pts-box-100.txt", "./tri-box-100.txt" )
	P = afficheDiGraph( G100, 0.02 )
	P += text("graphe",(0,0),rgbcolor=(0,0,0))
	P.show( axes = False )

	# print "Matrice poids pour le graphe à 100 sommets" #
	...
	print "Plus court chemin avec FloydWarshall pour le graphe à 100 sommets"
	print "du sommet 5 au sommet 17 avec poids E"
	print chemin(I100poidsE, 5, 17)
	print "du sommet 50 au sommet 44 avec poids E"
	print chemin(I100poidsE, 50, 44)
	V = afficheDiGraph( G100, 0.02 )
	V1 = afficheChemin( G100, I100poidsE, 5, 17, 0.02 ) 
	T = text("plus court 5-17",(0,0),rgbcolor=(0,0,0))
	(V+V1+T).show(axes=false)
	V1 = afficheChemin( G100, I100poidsE, 50, 44, 0.02 )
	T = text("plus court 50-44",(0,0),rgbcolor=(0,0,0))
	(V+V1+T).show(axes=false)

	# print "Affichage des distances sur le graphe à 100 sommets" #
	...
	print "Affichage des distances du parcours en profondeur sur le graphe à 100 sommets"
...
def test1000():
	# print "Affichage des distances sur le graphe à 1000 sommets" #
	...
	P += text("graphe distances E",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 
	# print "Affichage des distances du parcours en profondeur sur le graphe à 1000 sommets" #
...
# ce test est long à exécuter (plusieurs minutes)
def test10000(fct_poids = poidsE):
	# print "Affichage des distances sur le graphe à 10000 sommets" #
	G10000 = lireTriangulation( "./pts-box-10000.txt", "./tri-box-10000.txt" )
	P = afficheDiGraphDistance( G10000, 0, fct_poids, 0.02 )
	P += text("graphe distance",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false) 
	sys.setrecursionlimit(10000)
	# print "Affichage des distances du parcours en profondeur sur le graphe à 10000 sommets" #
	P = afficheDiGraphDistanceProfondeur( G10000, 5421, fct_poids, 0.02 )
	P += text("graphe profondeur",(0,0),rgbcolor=(0,0,0))
	P.show(axes=false)
